    ...
    Message: Clone,
{
    fn layout(&mut self, renderer: &Renderer, _bounds: Size) -> layout::Node {
        let limits = layout::Limits::new(Size::ZERO, _bounds)
            .width(Length::Fill)
            .height(Length::Fill);
        ...
        child = child.align(Alignment::Center, Alignment::Center, limits.max());

        layout::Node::with_children(limits.max(), vec![child]).move_to(self.position)
    }
